    ...
    <div class="bg-white rounded-xl shadow-sm p-3 lg:p-4 border border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg lg:text-xl font-bold text-gray-900"><?php echo e($totalContacts); ?></p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-3 lg:p-4 border border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg lg:text-xl font-bold text-gray-900"><?php echo e($totalVisitors); ?></p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-3 lg:p-4 border border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg lg:text-xl font-bold text-gray-900"><?php echo e($newContacts); ?></p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-3 lg:p-4 border border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg lg:text-xl font-bold text-gray-900"><?php echo e($totalImages); ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-3 lg:p-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('admin.contacts.index')); ?>" class="text-[10px] font-medium text-gold hover:underline">Ver todos</a>
        </div>
            ...
            <table class="w-full text-xs">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $recentContacts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contact): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 cursor-pointer" onclick="location.href='<?php echo e(route('admin.contacts.show', $contact)); ?>'">
                            <td class="px-3 lg:px-4 py-2.5 font-medium text-gray-900"><?php echo e(Str::limit($contact->name, 20)); ?></td>
                            <td class="px-3 lg:px-4 py-2.5 text-gray-500 hidden sm:table-cell"><?php echo e(Str::limit($contact->subject ?? '-', 30)); ?></td>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-3 lg:p-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('admin.visitors.index')); ?>" class="text-[10px] font-medium text-gold hover:underline">Ver todos</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-xs">
                <thead>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $recentVisitors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $visitor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 cursor-pointer" onclick="location.href='<?php echo e(route('admin.visitors.show', $visitor)); ?>'">
                            <td class="px-3 lg:px-4 py-2.5 font-medium text-gray-900"><?php echo e($visitor->ip_address ?? '-'); ?></td>
                            <td class="px-3 lg:px-4 py-2.5 text-gray-500 hidden sm:table-cell">
                                <?php if($visitor->city && $visitor->country): ?>
                                    <?php echo e($visitor->city); ?>, <?php echo e($visitor->country); ?>

                                <?php elseif($visitor->country): ?>
                            ...
                            <td class="px-3 lg:px-4 py-2.5 text-gray-500"><?php echo e($visitor->browser ?? '-'); ?></td>
                            <td class="px-3 lg:px-4 py-2.5 hidden md:table-cell">
                                <span class="px-1.5 py-0.5 text-[10px] rounded <?php echo e($visitor->device === 'Mobile' ? 'bg-blue-50 text-blue-600' : ($visitor->device === 'Tablet' ? 'bg-purple-50 text-purple-600' : 'bg-gray-50 text-gray-600')); ?>">
                                    <?php echo e($visitor->device ?? '-'); ?>

                                </span>
                            </td>
                            <td class="px-3 lg:px-4 py-2.5 font-medium text-gray-700"><?php echo e($visitor->pages_visited); ?></td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
const visitorLabels = <?php echo json_encode($visitorsByDay->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('d/m')), 15, 512) ?>;
const visitorData = <?php echo json_encode($visitorsByDay->pluck('count'), 15, 512) ?>;
...
const deviceLabels = <?php echo json_encode($visitorsByDevice->pluck('device'), 15, 512) ?>;
const deviceData = <?php echo json_encode($visitorsByDevice->pluck('count'), 15, 512) ?>;
...
const browserLabels = <?php echo json_encode($visitorsByBrowser->pluck('browser'), 15, 512) ?>;
const browserData = <?php echo json_encode($visitorsByBrowser->pluck('count'), 15, 512) ?>;
...
const countryLabels = <?php echo json_encode($visitorsByCountry->pluck('country'), 15, 512) ?>;
const countryData = <?php echo json_encode($visitorsByCountry->pluck('count'), 15, 512) ?>;
...
const osLabels = <?php echo json_encode($visitorsByOS->pluck('os'), 15, 512) ?>;
const osData = <?php echo json_encode($visitorsByOS->pluck('count'), 15, 512) ?>;
...
const pageLabels = <?php echo json_encode($topPages->pluck('page'), 15, 512) ?>;
const pageData = <?php echo json_encode($topPages->pluck('count'), 15, 512) ?>;
...
const contactLabels = <?php echo json_encode($contactsByDay->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('d/m')), 15, 512) ?>;
const contactData = <?php echo json_encode($contactsByDay->pluck('count'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\tshoot-angola\admin\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>
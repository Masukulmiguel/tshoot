...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-navy flex items-center gap-2">
                <i class="fas fa-pen-to-square text-gold"></i> Editar Publicação
            </h1>
            <p class="text-sm text-gray-500 mt-0.5">Actualizar a publicação: {{ $post->title }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ old('title', $post->title) }}" required class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gold/20 focus:border-gold outline-none transition">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-semibold text-gray-700 mb-1"><i class="fas fa-align-left text-gold mr-1"></i> Resumo</label>
            <textarea name="excerpt" rows="2" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gold/20 focus:border-gold outline-none resize-y transition">{{ old('excerpt', $post->excerpt) }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block text-sm font-semibold text-gray-700 mb-1"><i class="fas fa-file-alt text-gold mr-1"></i> Conteúdo</label>
            <textarea name="content" rows="10" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gold/20 focus:border-gold outline-none resize-y transition">{{ old('content', $post->content) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="text" name="author" value="{{ old('author', $post->author) }}" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gold/20 focus:border-gold outline-none transition">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="datetime-local" name="published_at" value="{{ old('published_at', $post->published_at?->format('Y-m-d\TH:i') ?? now()->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-gold/20 focus:border-gold outline-none transition">
            </div>
        </div>
